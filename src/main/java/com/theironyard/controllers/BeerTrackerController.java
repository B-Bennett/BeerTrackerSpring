...
public class BeerTrackerController {
    ...
    @PostConstruct
    public void init() throws InvalidKeySpecException, NoSuchAlgorithmException {  //is going to run after spring boots up. for default user
        User user = users.findOneByName("TheMan");
        if (user == null) {
            user = new User();
            user.name = "TheMan";
            user.password = PasswordHash.createHash("Man1");
            users.save(user);
        }
    }
